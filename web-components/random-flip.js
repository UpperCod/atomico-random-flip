import { customElement, useProp, useRef, useEvent, useEffect, h } from 'atomico';
import './web-cell.js';
import './web-grid.js';
...
const RandomFlip = props => {
  ...
  const ref = useRef();
  const dispatchDone = useEvent("done", {
    bubles: true,
    composed: true
  });
  const {
...
var randomFlip = customElement("random-flip", RandomFlip);
/*  Helpers */
...
const getWebGrid = ref => ref.current.shadowRoot.children[0]; // handle 'worldchange' event: a cell needs toggle
...
//# sourceMappingURL=random-flip.js.map
